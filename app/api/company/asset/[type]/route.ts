...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      const dataCol = type === "logo" ? "logo_data" : "stamp_data";
      const dr = type === "logo"
        ? await sql`SELECT logo_data AS d FROM companies WHERE id = ${company.id}`
        : await sql`SELECT stamp_data AS d FROM companies WHERE id = ${company.id}`;
