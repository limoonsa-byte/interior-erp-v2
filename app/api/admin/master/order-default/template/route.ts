...
import { ORDER_TEMPLATE_BASE64 } from "@/lib/order-template-base64";
...
/** 마스터 전용: 발주 기본 품목 엑셀 템플릿 다운로드 (DB 우선, 없으면 코드에 임베드된 템플릿 사용) */
export async function GET() {
  ...
  try {
    ...
    /* 테이블 없거나 조회 실패 → 임베드 템플릿 사용 */
  }
  const embed = ORDER_TEMPLATE_BASE64;
  if (!embed || typeof embed !== "string" || embed.length < 100) {
    console.error("order-default template: ORDER_TEMPLATE_BASE64 missing or too short");
