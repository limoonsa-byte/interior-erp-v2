  ...
  return (
    <div className="p-4 sm:p-6 max-w-xl mx-auto">
      ...
      <p className="text-sm text-gray-600 mb-6">
        앱처럼 사용하려면 아래에서 설치하거나, <strong>아래 주소를 공유</strong>하면 받는 사람이 클릭해서 설치할 수 있습니다.
      </p>

      {/* 설치 URL 공유 */}
      <div className="rounded-xl border-2 border-blue-200 bg-blue-50/50 p-4 mb-6">
        <p className="text-xs font-medium text-blue-800 mb-2">홈 추가 주소 (이 주소를 클릭하면 설치 페이지가 열립니다)</p>
      ...
      {installed ? (
        <div className="rounded-xl border border-green-200 bg-green-50 p-4 text-center text-green-800">
          ...
          <Link href="/consulting" className="mt-2 inline-block text-sm text-green-700 underline">
            메인으로 이동
          </Link>
        </div>
      ) : (
        <>
          <button
            ...
            className="w-full flex items-center justify-center gap-2 rounded-xl bg-blue-600 py-4 px-6 text-lg font-semibold text-white hover:bg-blue-700 active:bg-blue-800 disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          {installResult === "success" && (
            <p className="mt-3 text-center text-sm text-green-600">설치가 완료되었습니다. 바탕화면 또는 앱 목록에서 실행하세요.</p>
          )}
          {installResult === "dismissed" && (
            <p className="mt-3 text-center text-sm text-gray-500">설치를 취소하셨습니다. 다시 누르면 설치 창이 뜹니다.</p>
          )}

          {/* 수동 설치 방법 */}
          <div className="mt-8 rounded-xl border border-gray-200 bg-gray-50 p-4 text-sm text-gray-700">
            <h2 className="font-semibold text-gray-900 mb-3">직접 설치하는 방법</h2>
